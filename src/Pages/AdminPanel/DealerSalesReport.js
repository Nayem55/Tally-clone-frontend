...
const DealerSalesReport = () => {
  ...
  const organizeReportsHierarchically = (reports) => {
    ...
    return Object.entries(reportsByBelt).map(([belt, beltReports]) => {
      ...
      // Calculate rowspan for consecutive identical names
      const reportsWithRowspan = [];
      let currentName = null;
      let rowspanCount = 1;

      for (let i = 0; i < organizedReports.length; i++) {
        const report = organizedReports[i];
        const nextReport = organizedReports[i + 1];

        if (currentName === report.name) {
          rowspanCount++;
          reportsWithRowspan.push({ ...report, showName: false });
        } else {
          if (currentName !== null) {
            // Update the last report of the previous group with rowspan
            reportsWithRowspan[reportsWithRowspan.length - rowspanCount] = {
              ...reportsWithRowspan[reportsWithRowspan.length - rowspanCount],
          ...
          rowspanCount = 1;
          reportsWithRowspan.push({ ...report, showName: true });
        }

        // Handle the last report
        if (!nextReport && rowspanCount > 1) {
          reportsWithRowspan[reportsWithRowspan.length - rowspanCount] = {
            ...reportsWithRowspan[reportsWithRowspan.length - rowspanCount],
            rowspan: rowspanCount,
          };
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      <th className="p-2">Actual Sales (TP)</th>
