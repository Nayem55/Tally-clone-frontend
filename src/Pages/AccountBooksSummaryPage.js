...
export default function AccountBooksSummaryPage({ mode = "group" }) {
  ...
  useEffect(() => {
    async function loadReport() {
      ...
      try {
        ...
        setReport(
          response.data || { rows: [], searchRows: [], totals: {}, trail: [] },
        );
      } finally {
        setLoading(false);
      }
  ...
  useReportKeyboardNav(
    containerRef,
    [filteredRows, companyId, fromDate, toDate, mode, groupId],
    {
...
const creditRows = rowsForPdf
  ...
  .map((row) => ({
    ...
    const debitFinal =
      debitTotal < creditTotal ? debitTotal + closingDifference : debitTotal;
    const creditFinal =
      creditTotal < debitTotal ? creditTotal + closingDifference : creditTotal;
    ...
    const scopeLabel =
      breadcrumbLabel || (mode === "group" ? "Primary Groups" : "All Ledgers");
    ...
    function drawHeader() {
      ...
      doc.text(`Date : ${todayText}`, pageWidth - marginX, y, {
        align: "right",
      });
      ...
      doc.text(
        doc.splitTextToSize(paragraph1, pageWidth - marginX * 2),
        leftX,
        y,
      );
      y += 12;
      doc.text(
        doc.splitTextToSize(paragraph2, pageWidth - marginX * 2),
        leftX,
        y,
      );
      y += 15;
    ...
    for (let index = 0; index < maxRows; index += 1) {
      ...
      if (leftRow) {
        ...
        doc.text(
          formatPlainAmount(leftRow.amount, selectedCompany),
          amountRightLeft,
          y,
          {
            align: "right",
      ...
      if (rightRow) {
        ...
        doc.text(
          String(rightRow.particulars || "").slice(0, 34),
          rightX + 24,
          y,
        );
        doc.text(
          formatPlainAmount(rightRow.amount, selectedCompany),
          amountRightRight,
          y,
          {
            align: "right",
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      formatPlainAmount(debitTotal, selectedCompany),
      amountRightLeft,
      y,
      { align: "right" },
    );
    doc.text(
      formatPlainAmount(creditTotal, selectedCompany),
      amountRightRight,
      y,
      { align: "right" },
    );
    ...
    if (closingDifference > 0) {
      y += 10;
      doc.line(leftX, y - 6, amountRightLeft, y - 6);
      doc.line(rightX, y - 6, amountRightRight, y - 6);
      doc.setFont("helvetica", "normal");
      doc.text("Closing Balance", pageWidth / 14, y, { align: "left" });
      if (debitTotal < creditTotal) {
        doc.text(
          formatPlainAmount(closingDifference, selectedCompany),
          amountRightLeft,
          y,
          {
            align: "right",
      ...
      <div className="mx-auto max-w-[1460px] space-y-6">
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Value-only account hierarchy. Drill deeper with Enter and step
                back with Esc or Backspace.
              </p>
              {breadcrumbLabel ? (
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <SummaryCard
            icon={Layers3}
            title={
              mode === "group" ? "Visible Groups / Ledgers" : "Visible Ledgers"
            }
          ...
          <SummaryCard
            ...
            title="Credit"
            value={formatCurrencyAmount(
              Math.abs(Number(report.totals?.credit || 0)),
              selectedCompany,
            )}
            tone="text-rose-700"
          />
