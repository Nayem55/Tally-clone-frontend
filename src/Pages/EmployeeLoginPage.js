...
import { STORAGE_KEY, useActiveCompany } from "../Contexts/ActiveCompanyContext";
...
export default function EmployeeLoginPage() {
  ...
  if (
    storedUser &&
    String(storedUser.companyId || "") === String(window.localStorage.getItem(STORAGE_KEY) || "")
  ) {
  ...
  async function handleEmployeeLogin(event) {
    ...
    try {
      ...
      window.localStorage.setItem(
        "attendance-user",
        JSON.stringify({ _id: user.attendance_id || user.employeeId || user._id }),
      );
      navigate(targetPath, { replace: true });
    } catch (error) {
      setErrorMessage(
      ...
      <div className="mx-auto w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Select a company first. If employee access is already set up there, we’ll ask for username and password next.
          </p>
        </div>

        {companies.length === 0 && !loadingCompanies ? (
          <div className="mt-8 rounded-2xl border border-blue-200 bg-blue-50/70 p-5">
            <h2 className="text-lg font-semibold text-slate-900">No company found yet</h2>
            <p className="mt-2 text-sm text-slate-600">
              Create your first company to start the accounting workspace. After that, you can come back here and open it directly.
            </p>
            <div className="mt-4">
              <Link
                to="/company/create"
        ...
        <form className="mt-8 space-y-5" onSubmit={needsEmployeeLogin ? handleEmployeeLogin : handleOpenCompany}>
